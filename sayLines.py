...
for line in lines:
    if "-->" in line:
        ...
        if len(paragraph) > 0 and count > 0 and not path.exists(targetFile + ".m4a"):
            ...
            concatString = concatString + "[" + str(fileCount) + ":a]"
            fileCount = fileCount + 1
            subprocessArray.extend(["-filter_complex", concatString + "concat=n=" + str(fileCount) + ":v=0:a=1", targetFilePrefix + ".mp4"])
            subprocess.call(subprocessArray)
            os.system("rm " + targetFilePrefix + "~.mp4")
        paragraph = ""
        count = count + 1
    else:
